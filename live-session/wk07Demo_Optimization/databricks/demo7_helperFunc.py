...
def compareDescent(X, y, hist1, hist2, labels):
    ...
    ax.set_xlabel('w_0 (parameter)')
    ax.set_ylabel(f'w_1 (bias)')
    ...
    # add contour plots
    w0_min = min(hist1.T[0].min(), hist2.T[0].min()) - 0.15
    w1_min = min(hist1.T[1].min(), hist2.T[1].min()) - 0.15
    w0_max = max(hist1.T[0].max(), hist2.T[0].max()) + 0.15
    w1_max = max(hist1.T[1].max(), hist2.T[1].max()) + 0.15
    plotLossContours(ax, X, y, w0_min, w0_max, w1_min, w1_max)

    # include legend
    plt.legend()
    
    # No longer needed by Databricks
    #return plt.show()
